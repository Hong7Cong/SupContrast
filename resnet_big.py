...
def sets_to_tensor(list_of_sets):
    return torch.tensor([list(s) for s in list_of_sets], dtype=torch.float32)
# def pairwise_combinations(lst):
#     return list(combinations(lst, 2))

def pairwise_combinations_tensor(lst):
    pairs = list(product(lst, repeat=2))
    pairs = [(p1.tolist(), p2.tolist()) for p1, p2 in pairs]  # Convert tensors to lists
...
model_dict = {
    ...
    'vit_small': [torch.hub.load('facebookresearch/dino:main', 'dino_vits16'), 384],
...
class CrossAttViT(nn.Module):
    def __init__(self, name='vit_small', head='mlp', feat_dim=128, method="SupCon"):
        super(CrossAttViT, self).__init__()
        model, dim_in = model_dict['vit_small']  
    ...
    def forward(self, images):
        bsz = images.shape[0]
        pairs = pairwise_combinations_tensor(images).cuda()
        # inputs = sets_to_tensor(pairs)
        # print(pairs.device, self.encoder.device)
        # assert False
        
        img1_embs = self.encoder.get_intermediate_layers(pairs[:,0,:], n=1)[0]
        img2_embs = self.encoder.get_intermediate_layers(pairs[:,1,:], n=1)[0]
        # img1_emb, img2_emb = torch.split(imgs_emb, [bsz, bsz], dim=0)
                    
        img1_att, img2_att = self.cross_atten(img1_embs, img2_embs, mask=None, context_mask=None)
        # print(img1_att.shape, img1_embs.shape)
        # print(pairdot.shape)
        # assert False
        feat_img1 = F.normalize(self.head(img1_att), dim=2, eps=1e-8)
        feat_img2 = F.normalize(self.head(img2_att), dim=2, eps=1e-8)

        pairdot = torch.sum(torch.mul(feat_img1[:,0,:], feat_img2[:,0,:]), dim=1).view(bsz, bsz)
